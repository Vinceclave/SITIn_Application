...
/* Example of a style to hide the PC Availability section if needed */
/* #pcAvailabilitySection {
    display: none;
} */
</script>
        ...
        function displayReservations(reservations) {
            ...
            reservations.forEach(reservation => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">${reservation.purpose}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                         ${reservation.purpose ? htmlspecialchars(reservation.purpose) : '<span class="text-gray-500">N/A</span>'}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusClass}">
                            ${reservation.status.charAt(0).toUpperCase() + reservation.status.slice(1)}                            
        ...
        function updateReservationStatus(reservationId, status) {
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    try {
                        const response = JSON.parse(xhr.responseText);
                        if (response.success) { // Assuming the response has a 'success' key
                            // Refresh data after successful update
